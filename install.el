...
(defvar make-sdic-lisp-directory (expand-file-name "../../site-lisp/sdic" data-directory)
...
(defun make-sdic ()
  ...
  ;; インストール済の辞書があるか調べる
  (let ((eiwa (catch 'found-eiwa
		(mapcar (lambda (basename)
			  (and (file-exists-p (expand-file-name basename make-sdic-dict-directory))
			       (throw 'found-eiwa basename)))
			(list "gene.sdic" "ejdict.sdic" "gene.dic" "ejdict.dic"))
		(file-exists-p "gene.txt")))
	(waei (catch 'found-waei
		(mapcar (lambda (basename)
			  (and (file-exists-p (expand-file-name basename make-sdic-dict-directory))
			       (throw 'found-waei basename)))
			(list "jedict.sdic" "jgene.sdic" "jedict.dic" "jgene.dic"))
		(file-exists-p "edict"))))
    (make-sdic-sdic_el (if (eq eiwa t) "gene.sdic" eiwa) (if (eq waei t) "jedict.sdic" waei))
  ...
  (let ((in-file (expand-file-name "sdic.el.in" make-sdic-src-directory))
	...
	(unwind-protect
	    (progn
	      ...
	      (delete-region (goto-char (match-beginning 0)) (match-end 0))
	      (insert (expand-file-name eiwa make-sdic-dict-directory))
	      (goto-char (point-min))
	      (search-forward "@WAEI_DICTIONARY_PATH@")
	      (delete-region (goto-char (match-beginning 0)) (match-end 0))
	      (insert (expand-file-name waei make-sdic-dict-directory))
	      (make-sdic-write-file out-file))
	  (kill-buffer buf))))))
